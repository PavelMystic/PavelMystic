...
We can write the expected value of KL divergence between conditional probability density function $$h(\mathbf{x}_t \vert \mathcal{Z}_t)$$ of track state $$\mathbf{x}_t$$ (e.g. position and velocity) given measurements $$\mathcal{Z}_t$$ (e.g. position) up to time $$t$$ and conditional probability density function $$h(\mathbf{x}_t \vert \mathcal{Z}_{t-1})$$ of track state $$\mathbf{x}_t$$ given measurements $$\mathcal{Z}_{t-1}$$ up to time $$t-1$$ as follows

$$
\mathsf{E}[D_{KL} (h(\mathbf{x}_t \vert \mathcal{Z}_t) \vert \vert h(\mathbf{x}_t \vert \mathcal{Z}_{t-1}))] = \int_{\mathbb{R}^m} f^*(\mathbf{z}_t \vert \mathcal{Z}_{t-1})  D_{KL} (h(\mathbf{x}_t \vert \mathcal{Z}_t) \vert \vert h(\mathbf{x}_t \vert \mathcal{Z}_{t-1})) \, \mathrm{d} \mathbf{z}_t,
$$

where $$f^*(\mathbf{z}_t \vert \mathcal{Z}_{t-1})$$ is predictive probability density function of measurements and $$\mathbf{z}_t$$ is the measurement at time $$t$$. \\
...
$$
\begin{aligned}
\label{eq_mch_info_gain}
    e_t(\mathbf{X}, m) = \begin{cases} \frac{\sum_{j \in J_m}\mathsf{E}[D_{KL}^{j,t}]}{|J_m|}  &\mbox{if } |J_m| \neq 0 \\ 0 &\mbox{otherwise} \end{cases}.
\end{aligned}
$$
